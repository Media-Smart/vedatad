...
at::Tensor nms_cpu_kernel(const at::Tensor& dets, const float threshold) {
  ...
  auto x2_t = dets.select(1, 1).contiguous();
  auto scores = dets.select(1, 2).contiguous();

  at::Tensor areas_t = x2_t - x1_t;
  ...
  for (int64_t _i = 0; _i < ndets; _i++) {
    ...
    for (int64_t _j = _i + 1; _j < ndets; _j++) {
      ...
      auto inter = std::max(static_cast<scalar_t>(0), xx2 - xx1);
...
at::Tensor soft_nms_cpu_kernel(const at::Tensor& dets, const float threshold,
  ...
  auto x1_t = dets.select(1, 0).contiguous();
  auto x2_t = dets.select(1, 1).contiguous();
  auto scores_t = dets.select(1, 2).contiguous();
  ...
  for (int64_t i = 0; i < ndets; i++) {
    ...
    // add max segment as a detection
    x1[i] = x1[max_pos];
    x2[i] = x2[max_pos];
    ...
    // swap ith segment with position of max segment
    x1[max_pos] = ix1;
    x2[max_pos] = ix2;
    ...
    pos = i + 1;
    // NMS iterations, note that N changes if detection segments fall below
    // threshold
    while (pos < ndets) {
      ...
      // if segment score falls below threshold, discard the segment by
      // swapping with last segment update N
  ...
  result[1] = x2_t.slice(0, 0, ndets);
  result[2] = scores_t.slice(0, 0, ndets);
  result[3] = inds_t.slice(0, 0, ndets);
